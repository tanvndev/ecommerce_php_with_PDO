...
(function ($) {
  ...
  var echeader = $('#ec-main-menu-desk');
  ...
  var toggleHeader = function (ecdirection, eccurScroll) {
    if (ecdirection === 1 && eccurScroll > 52) {
      // scrolled down and beyond the initial header height
      echeader.addClass('menu_fixed');
      echeader.removeClass('menu_fixed_up');
      ecprevDirection = ecdirection;
    } else if (ecdirection === 2) {
      // scrolled up
      echeader.addClass('menu_fixed_up');
      echeader.removeClass('menu_fixed');
      ecprevDirection = ecdirection;
    }
  };

  $(window).on('scroll', function () {
    var $stickyHeaderNextSec = $('.sticky-header-next-sec');
    var distance = $stickyHeaderNextSec.length
      ? $stickyHeaderNextSec.offset().top
      : 0;

    if ($(window).scrollTop() <= distance + 50) {
      // scrolled to or above the distance
      echeader.removeClass('menu_fixed menu_fixed_up');
    } else {
      // scrolled below the distance
      checkScroll();
    }
  });
